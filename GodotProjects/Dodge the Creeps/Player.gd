...
func _process(delta):      # Called every frame
	...
	if velocity.length() > 0: # Length seems like its the combination of both the x and y: (1, 1).length = 2
		...
		$AnimatedSprite2D.play()  # '$' is shorthand for 'get_node()' for the relative path from current node so...
	else:                       # ...
		$AnimatedSprite2D.stop()  # that <-- is the same as get_node("AnimatedSprite2D").play()
	
	# Clamping player (AKA restruicting to a given range (prevent it from leaving screen))
	position += velocity * delta
	position.x = clamp(position.x, 28, screen_size.x - 28)   # super.clamp(value, min, max)
