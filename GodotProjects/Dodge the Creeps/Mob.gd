...
func _ready():
	var mob_types = $AnimatedSprite2D.frames.get_animation_names()  # Getting list of names from frames ["walk", "swim", "fly"]
	$AnimatedSprite2D.animation = mob_types[randi() % mob_types.size()]  # Setting animation as a random pick in the list ^
